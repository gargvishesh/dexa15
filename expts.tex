...
by conducting additional experiments with reduced DRAM sizes. The write estimators were also validated by comparing them with the actual writes incurred during query executions. Both 
these results are covered in the Tech Report \textbf{[Citation to be included]}.
\begin{figure*}[htpb]
\centering
\subfloat[Q13]{
...
$size_{j}$ are $2 \times 10^5$, $8$, $120$ and $8$, respectively. 
For the PCM-conscious version, the estimated writes are given by Equation \ref{eq:ht_pcm},
